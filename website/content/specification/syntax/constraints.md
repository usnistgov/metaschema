...
The `@target` attribute of a `matches` constraint can be matched based on its value and a regular expression in the form of `@regex` that the node matches on. The `@regex` syntax includes character classes.
