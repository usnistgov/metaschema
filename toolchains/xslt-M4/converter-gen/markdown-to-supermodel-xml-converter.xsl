...
<!-- outside blocks marked as code, we split wherever we have double LF -->
    ...
    <xsl:template match="tr[m:is-table-row-demarcator(string(.))]" priority="5" mode="make-row"/>
    ...
    <xsl:template match="tr[some $f in (following-sibling::tr) satisfies m:is-table-row-demarcator(string($f))]" mode="make-row">
        <tr>
            <xsl:for-each select="tokenize(string(.), '\s*\|\s*')[not(position() = (1,last())) ]">
                <th>
                    <xsl:value-of select="."/>
    ...
    <xsl:template mode="build-structures" match="p[@header-level]">
        ...
        <xsl:element name="h{$level}" namespace="http://csrc.nist.gov/ns/oscal/metaschema/1.0/supermodel">
            <xsl:value-of select="."/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template mode="build-structures" match="list" name="nest-lists">
        ...
        <xsl:element name="{ $group[1]/@type }" namespace="http://csrc.nist.gov/ns/oscal/metaschema/1.0/supermodel">
            <xsl:for-each-group select="$group" group-starting-with="li[(@level = $level) or not(@type = preceding-sibling::*[1]/@type)]">
                <xsl:choose>
