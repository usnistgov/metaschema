...
<!-- 
    ...
    <xsl:key name="definition-by-name" match="METASCHEMA/define-flag | METASCHEMA/define-field | METASCHEMA/define-assembly" use="@name"/>
...
<!-- pull all schemas together
    ...
    <xsl:template mode="keep-eligible" priority="11"
        match="METASCHEMA/define-assembly[exists(root-name)]">
        <xsl:if test="$verbose">
            <xsl:message expand-text="true">KEEPING definition for '{ @name }' assembly from { ancestor::METASCHEMA[1]/@module }</xsl:message>
        </xsl:if>
    ...
    <xsl:template mode="keep-eligible" priority="10"
        match="METASCHEMA/define-field[exists(key('field-references',@name))]
        | METASCHEMA/define-flag[exists(key('flag-references',@name))]
        | METASCHEMA/define-assembly[exists(key('assembly-references',@name))]">
        <xsl:if test="$verbose">
    ...
    <xsl:variable name="composed-metaschema" as="document-node()">
        <xsl:variable name="all-references" as="xs:string*">
            <xsl:apply-templates mode="collect-references" select="$eligible/METASCHEMA/define-assembly[matches(root-name,'\i\c*$')]">
...
<!-- Mode 'collect references' collects a set of strings naming definitions
    ...
    <xsl:template match="define-assembly" mode="collect-references">
        ...
        <xsl:if test="not(@name = $ref-stack)">
            <xsl:sequence select="@name, descendant::flag/@ref"/>
    ...
    <xsl:template priority="5" match="model[exists(@ref)]" mode="collect-references">
    ...
    <xsl:template match="model | model//*" mode="collect-references">
        <xsl:param name="ref-stack" tunnel="yes"/>
        <xsl:apply-templates mode="#current">
            <xsl:with-param tunnel="true" name="ref-stack" select="$ref-stack,@ref"/>
        </xsl:apply-templates>  
    </xsl:template>
