...
<!-- 
    Run on a metaschema or metaschema fragment to produce CSS for styling it.
    Will also work over HTML projected with 'class' capturing the OSCAL type.
  -->
  ...
  <xsl:variable name="divs"     select="//define-assembly | //define-field[@as-type='markup-multiline']"/>
  ...
  <xsl:template match="METASCHEMA">
    ...
    <xsl:for-each select="define-field[@as-type='markup-multiline']">
      <xsl:text expand-text="true">{ $lf2 }{ m:use.name(.) } &gt; *:first-child, .{ m:use.name(.) } &gt; *:first-child {{ margin-top: 0ex }}</xsl:text>
    </xsl:for-each>
    <xsl:call-template name="css-literal"/>
  </xsl:template>
  ...
  <xsl:template priority="2" match="define-assembly | define-field" mode="display-rule">
    ...
    <xsl:text expand-text="true">{ $lf } {{ display: block; </xsl:text>
    <xsl:if test="empty(root-name)" expand-text="true"> margin-left: 1rem; padding-left: 1rem; border-left: thin solid grey</xsl:if>
    <xsl:text> }</xsl:text>
  </xsl:template>
  
  <xsl:variable name="label.color">#0D4F7B</xsl:variable>
  
  <xsl:variable name="label.properties" expand-text="true">font-family: sans-serif; font-size: 0.83rem; font-weight: bold; background-color: { $label.color }; color: white; padding: 0ex 0.4ex; margin: 0ex 0.3ex;</xsl:variable>
